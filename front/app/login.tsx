...
export default function LoginScreen() {
  ...
  // Refs pour gérer le focus entre les champs
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const isUnverified =
        error?.message === "ACCOUNT_UNVERIFIED" ||
        (error?.response &&
          error.response.data &&
          error.response.data.unverified);
      if (isUnverified) {
        ...
        router.replace({
          pathname: "/verify-otp",
          params: { email: targetEmail, autoResend: "true" },
        });
        return;
      }

      const normal = normalizeErrorMessage(
      ...
      try {
        ...
        if (
          n.includes("requête invalide") ||
          n.includes("request failed") ||
          n.includes("request invalid") ||
          /status code 400/.test(n) ||
          /timeout/.test(n)
        ) {
          friendly =
      ...
      console.log(
        "[Login] setting errorMsg (normal/friendly):",
        normal,
        "/",
        friendly,
      );
  ...
  return (
    <>
      <AuthLayout topInset={Platform.OS === "android" ? 40 : 60}>
              ...
              <View style={styles.form}>
                ...
                {showUnverifiedActions && (
                  <View style={styles.inlineActions}>
                    <TouchableOpacity
                      ...
                      onPress={async () => {
                        ...
                        try {
                          ...
                          setErrorMsg(
                            "Code renvoyé. Vérifiez votre téléphone.",
                          );
                        } catch (e: any) {
                      ...
                      style={[
                        styles.smallBtn,
                        resending && styles.smallBtnDisabled,
                      ]}
                    >
                    ...
                    >
                      <Text style={styles.outlineLinkText}>
                        Vérifier maintenant
                      </Text>
                    </TouchableOpacity>
                  </View>
