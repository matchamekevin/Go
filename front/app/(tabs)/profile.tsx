...
export default function ProfileTab() {
  const { user, logout, updateUserProfile, isLoading, refreshAuth } = useAuth();
  ...
  useEffect(() => {
    if (user) {
      console.log("👤 Mise à jour des données du profil avec:", user);
      setUserProfile((prev) => ({
        ...prev,
        name: user.name || "Utilisateur",
        email: user.email || "",
  ...
  const handleSendContact = async () => {
    if (!contactMessage.trim()) {
      Alert.alert(
        "Message vide",
        "Veuillez écrire votre message avant d'envoyer.",
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        "Message envoyé",
        `Votre message a été envoyé à l'équipe support. Nous vous répondrons bientôt.`,
      );
      setContactMessage("");
            ...
            { text: "Annuler", style: "cancel" },
            {
              ...
              onPress: () => {
                ...
                Linking.openURL(mailto).catch((e) =>
                  console.error("Erreur ouverture mailto", e),
                );
              },
            },
          ],
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      Alert.alert(
        "Erreur",
        "Échec de la mise à jour du profil. Vérifiez votre connexion.",
      );
    }
  };
        ...
        <View style={styles.header}>
          <View style={styles.profileSection}>
            ...
            <TouchableOpacity
              ...
              onPress={() => setEditAvatarModalOpen(true)}
            >
              <Ionicons
                name="create-outline"
                size={20}
            ...
            <View style={styles.modalCardLarge}>
              ...
              <ScrollView style={{ marginTop: theme.spacing.md }}>
                <Text style={styles.legalText}>
                  ...
                  Go Transport vous aide à trouver et réserver des trajets
                  inter-urbains et urbains depuis votre téléphone. Principales
                  fonctionnalités :{"\n\n"}• Recherche d'itinéraires et horaires
                  en temps réel.{"\n"}• Achat sécurisé de billets et gestion des
                  réservations.{"\n"}• Affichage et présentation du billet via
                  QR code.{"\n"}• Historique des voyages et suivi des points
                  fidélité.{"\n"}• Gestion de votre compte (profil, moyens de
                  paiement, préférences).{"\n\n"}
                  FAQ et assistance rapide{"\n\n"}Q : Comment réinitialiser mon
                  mot de passe ?{"\n"}R : Allez sur « Mot de passe oublié »,
                  saisissez votre email puis suivez le code reçu par email pour
                  réinitialiser votre mot de passe.{"\n\n"}Q : Comment acheter
                  un billet ?{"\n"}R : Recherchez votre trajet, sélectionnez
                  l'horaire désiré, choisissez le nombre de passagers, puis
                  procédez au paiement via les moyens proposés.{"\n\n"}Q : Quels
                  moyens de paiement sont acceptés ?{"\n"}R : Les moyens
                  disponibles sont listés dans « Moyens de paiement ». Les modes
                  varient selon l'opérateur (carte, mobile money, etc.).{"\n\n"}
                  Q : Puis-je annuler ou me faire rembourser ?{"\n"}R : Les
                  politiques d'annulation et de remboursement dépendent de
                  l'opérateur de transport. Contactez le support avec votre
                  numéro de billet pour assistance.{"\n\n"}Q : Comment contacter
                  le support ?{"\n"}R : Utilisez la section « Nous contacter »
                  dans votre profil ou écrivez à notre équipe via l'adresse
                  fournie ; décrivez votre problème et joignez les informations
                  utiles (email, numéro de billet).{"\n\n"}
                  Pour toute question non couverte ici, envoyez-nous un message
                  via le formulaire de contact — nous revenons généralement sous
                  48h.
                </Text>
              </ScrollView>
            ...
            <View style={styles.modalCardLarge}>
              ...
              <ScrollView style={{ marginTop: theme.spacing.md }}>
                {isLegalOpen.tab === "privacy" ? (
                  <Text style={styles.legalText}>
                    Politique de confidentialité{"\n\n"}
                    Nous recueillons et utilisons vos données personnelles
                    uniquement pour fournir et améliorer nos services. Les
                    informations collectées peuvent inclure votre nom, adresse
                    e‑mail, numéro de téléphone et données de trajet. Nous ne
                    partageons pas vos données avec des tiers à des fins
                    commerciales sans votre consentement explicite. Vous pouvez
                    demander la suppression ou l'accès à vos données en
                    contactant notre support.
                    {"\n\n"}Cette politique s'applique à l'ensemble des
                    fonctionnalités de l'application Go Transport. Pour toute
                    question relative à la confidentialité, écrivez-nous via la
                    section « Nous contacter ».
                  </Text>
                ) : (
                  <Text style={styles.legalText}>
                    Conditions d'utilisation{"\n\n"}
                    Bienvenue sur Go Transport. En utilisant cette application,
                    vous acceptez nos conditions d'utilisation suivantes :
                    utiliser le service conformément à la loi, fournir des
                    informations exactes, et ne pas abuser des fonctionnalités
                    (fraude, revente de billets, etc.). Nous nous réservons le
                    droit de suspendre ou supprimer un compte en cas de
                    violation.
                    {"\n\n"}Les informations fournies par l'application sont
                    données à titre indicatif. Les horaires et tarifs peuvent
                    être modifiés par les opérateurs. Nous déclinons toute
                    responsabilité en cas d'erreurs ou de changements de
                    dernière minute indépendants de notre volonté.
                  </Text>
                )}
              </ScrollView>
