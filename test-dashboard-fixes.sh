...
if curl -s http://localhost:3000/health > /dev/null; then
    ...
    echo "❌ Serveur backend non accessible - démarrer avec 'npm run dev' dans /back"
    exit 1
fi

# Tester l'endpoint des statistiques du dashboard
echo "2. Test de l'endpoint /admin/dashboard/stats..."
RESPONSE=$(curl -s -H "Authorization: Bearer test-token" http://localhost:3000/admin/dashboard/stats)
...
CHART_RESPONSE=$(curl -s -H "Authorization: Bearer test-token" "http://localhost:3000/admin/dashboard/chart-data?type=revenue&period=30d")
...
ACTIVITY_RESPONSE=$(curl -s -H "Authorization: Bearer test-token" http://localhost:3000/admin/dashboard/recent-activity)
