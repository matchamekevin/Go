...
import { SotralLine } from '../types/api';
  ...
  const loadData = async () => {
    try {
      ...
      if (linesRes.ok) {
        ...
        setAllLinesForSelection(apiLinesData);
        setApiError(null);
      } else {
        setAllLinesForSelection([]);
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {lines.map((line) => {
                  ...
                  const tarifs = isStudentLine ? '100' : 
                    line.distance_km <= 10 ? '100, 150' :
                    line.distance_km <= 15 ? '100, 150, 200' :
                    line.distance_km <= 20 ? '100, 150, 200, 250' :
                    '100, 150, 200, 300';
                  ...
                  {lines.filter(line => line.is_active).map(line => (
                    <label key={line.id} className="flex items-center">
                      <input
                        ...
                        className="mr-2"
                      />
                      <span className="text-sm">Ligne {line.line_number} - {line.name}</span>
                    </label>
                  ))}
                </div>
              </div>
