...
export class AuthService {
  static async register(data: {
    ...
    if (!data || !data.email || !data.name || !data.password || !data.phone) {
      throw new Error(
        "Champs manquants pour l'inscription : email, name, password et phone sont requis",
      );
    }
    ...
    if (!isValidTgPhone(localPhone)) {
      throw new Error(
        "Numéro de téléphone invalide. Utilisez un numéro togolais (ex: +228 XX XX XX XX) pour YAS ou Moov.",
      );
    }
    ...
    sendEmail(user.email, emailSubject, emailText, emailHtml).catch((error) => {
      console.error(
        `[AuthService] Erreur envoi email pour ${user.email}:`,
        error,
      );
    ...
    return {
      ...
      message: "Utilisateur créé. Vérifiez votre email pour l OTP.",
    };
  }

  static async verifyEmailOTP(email: string, otp: string) {
    console.log(
      "[AuthService.verifyEmailOTP] attempt for email:",
      email,
      "otp:",
      otp,
    );
    ...
    console.log(
      "[AuthService.verifyEmailOTP] user found:",
      !!user,
      user
    ...
    console.log(
      "[AuthService.verifyEmailOTP] latest OTP record:",
      !!record,
      record
        ? { id: record.id, otp: record.otp, expires_at: record.expires_at }
        : null,
    );
    ...
    if (record.otp !== otp) {
      console.log(
        "[AuthService.verifyEmailOTP] OTP mismatch: provided",
        otp,
        "expected",
        record.otp,
      );
  ...
  static async resendEmailOTP(email: string) {
    ...
    const user = await UserRepository.findByEmail(email);
    console.log(
      "[AuthService.resendEmailOTP] user found:",
      !!user,
      user
        ? { id: user.id, email: user.email, is_verified: user.is_verified }
        : null,
    ...
    if (user.is_verified) {
      console.log(
        "[AuthService.resendEmailOTP] user already verified:",
        user.id,
      );
    ...
    console.log("[AuthService.resendEmailOTP] created OTP record:", {
      id: record.id,
      otp,
      expiresAt,
    });
    ...
    sendEmail(user.email, emailSubject, emailText, emailHtml).catch((error) => {
      console.error(
        `[AuthService.resendEmailOTP] Erreur envoi email pour ${user.email}:`,
        error,
      );
    });
