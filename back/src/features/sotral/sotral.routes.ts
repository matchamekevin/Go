...
router.get(
  "/lines/category/:categoryId",
  sotralController.getLinesByCategory.bind(sotralController),
);
router.get("/stops", sotralController.getAllStops.bind(sotralController));
router.get(
  "/lines/:id/stops",
  sotralController.getStopsByLine.bind(sotralController),
);

// Types de tickets et tarification
router.get(
  "/ticket-types",
  sotralController.getAllTicketTypes.bind(sotralController),
);
router.post(
  "/calculate-price",
  sotralController.calculatePrice.bind(sotralController),
);

// Achat de tickets (authentification optionnelle pour l'instant)
router.post(
  "/purchase",
  sotralController.purchaseTicket.bind(sotralController),
);
router.get("/my-tickets", authMiddleware as any, sotralController.getMyTickets);
router.delete(
  "/my-tickets/:id",
  authMiddleware as any,
  sotralController.cancelUserTicket,
);

// Tickets générés par l'admin (publics pour le mobile)
router.get(
  "/generated-tickets",
  sotralController.getGeneratedTickets.bind(sotralController),
);

// Gestion des tickets admin (privé)
router.delete(
  "/tickets/:id",
  sotralController.deleteTicketAdmin.bind(sotralController),
);
