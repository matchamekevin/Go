...
export class AdminController {
  ...
  static async getUserStats(_req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const verifiedRes = await pool.query(
        "SELECT COUNT(*)::int as verified FROM users WHERE is_verified = true",
      );
      const suspendedRes = await pool.query(
        "SELECT COUNT(*)::int as suspended FROM users WHERE COALESCE(is_suspended,false) = true",
      );
      return res
        .status(200)
        .json({
          ...
          data: {
            ...
            verified: verifiedRes.rows[0].verified,
            suspended: suspendedRes.rows[0].suspended,
          },
        });
    } catch (err: any) {
      ...
      return res
        .status(500)
        .json({ success: false, error: "Erreur calcul stats utilisateurs" });
    }
  }
  ...
  static async getAllUsers(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      if (role && role !== "all") {
        if (role === "verified") where += " AND is_verified = true";
          ...
          try {
            ...
            console.warn(
              "[getAllUsers] Colonne is_suspended inexistante, filtre ignoré",
            );
          }
        }
      }
      ...
      let selectColumns =
        "id, email, name, phone, is_verified::boolean, created_at, COALESCE(updated_at, created_at) as updated_at";
  ...
  static async updateUser(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const r = await pool.query(
        "UPDATE users SET name = COALESCE($1,name), phone = COALESCE($2,phone), is_verified = COALESCE($3,is_verified), updated_at = NOW() WHERE id = $4 RETURNING id, email, name, phone, is_verified, COALESCE(is_suspended,false) as is_suspended, created_at, COALESCE(updated_at, created_at) AS updated_at",
  ...
  static async toggleUserStatus(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      return res
        .status(500)
        .json({ success: false, error: "Erreur lors du changement de statut" });
    }
  }
  ...
  static async getSuspendedUsers(_req: AuthenticatedRequest, res: Response) {
    try {
      ...
      return res
        .status(200)
        .json({ success: true, data: data.rows, count: data.rows.length });
    } catch (err: any) {
      ...
      return res
        ...
        .json({
          success: false,
          error: "Erreur lors de la récupération des comptes suspendus",
        });
    }
  }
  ...
  static async toggleUserSuspension(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const currentUser = await pool.query(
        "SELECT COALESCE(is_suspended, false) as is_suspended FROM users WHERE id = $1",
        [id],
      );
      ...
      const r = await pool.query(
        `UPDATE users SET is_suspended = $1, updated_at = NOW() WHERE id = $2 RETURNING ${returningClause}`,
        [newSuspended, id],
      );
      ...
      return res
        ...
        .json({
          success: false,
          error: "Erreur lors du changement de statut de suspension",
        });
    }
  }
  ...
  static async getRouteStats(_req: AuthenticatedRequest, res: Response) {
    try {
      ...
      console.error("[AdminController.getRouteStats]", err);
      return res
        .status(500)
        .json({ success: false, error: "Erreur routes stats" });
    }
  }

  static async getAllRoutes(_req: AuthenticatedRequest, res: Response) {
    try {
      const r = await pool.query("SELECT * FROM routes ORDER BY name ASC");
      return res.status(200).json({ success: true, data: r.rows });
    } catch (err: any) {
      console.error("[AdminController.getAllRoutes]", err);
      return res
        .status(500)
        .json({ success: false, error: "Erreur récupération routes" });
    }
  }
  static async createRoute(_req: AuthenticatedRequest, res: Response) {
    return res.status(201).json({ success: true, data: {} });
  }
  static async updateRoute(_req: AuthenticatedRequest, res: Response) {
    return res.status(200).json({ success: true, data: {} });
  }
  static async deleteRoute(_req: AuthenticatedRequest, res: Response) {
    return res.status(200).json({ success: true });
  }

  // ---------- PRODUCTS ----------
  static async getAllProducts(_req: AuthenticatedRequest, res: Response) {
    try {
      const p = await pool.query(
        "SELECT * FROM sotral_ticket_types ORDER BY name ASC",
      );
      return res.status(200).json({ success: true, data: p.rows });
    } catch (err: any) {
      console.error("[AdminController.getAllProducts]", err);
      return res
        .status(500)
        .json({ success: false, error: "Erreur récupération produits" });
    }
  }
  static async createProduct(_req: AuthenticatedRequest, res: Response) {
    return res.status(201).json({ success: true, data: {} });
  }
  static async updateProduct(_req: AuthenticatedRequest, res: Response) {
    return res.status(200).json({ success: true, data: {} });
  }
  static async deleteProduct(_req: AuthenticatedRequest, res: Response) {
    return res.status(200).json({ success: true });
  }

  // ---------- TICKETS ----------
  static async getAllTickets(_req: AuthenticatedRequest, res: Response) {
    try {
      const t = await pool.query(
        "SELECT * FROM tickets ORDER BY created_at DESC LIMIT 100",
      );
      return res.status(200).json({ success: true, data: t.rows });
    } catch (err: any) {
      console.error("[AdminController.getAllTickets]", err);
      return res
        .status(500)
        .json({ success: false, error: "Erreur récupération tickets" });
    }
  }
  static async updateTicketStatus(_req: AuthenticatedRequest, res: Response) {
    return res.status(200).json({ success: true });
  }
  ...
  static async getDashboardStats(_req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const totalUsers = await pool
        ...
        .catch(() => ({ rows: [{ total: 0 }] }));
      const verifiedUsers = await pool
        .query(
          "SELECT COUNT(*)::int as total FROM users WHERE is_verified = true",
        )
        .catch(() => ({ rows: [{ total: 0 }] }));
      const newUsersMonth = await pool
        .query(
          "SELECT COUNT(*)::int as total FROM users WHERE created_at >= CURRENT_DATE - INTERVAL '30 days'",
        )
        .catch(() => ({ rows: [{ total: 0 }] }));
      ...
      try {
        totalTickets = await pool.query(
          "SELECT COUNT(*)::int as total FROM sotral_tickets",
        );
        usedTickets = await pool.query(
          "SELECT COUNT(*)::int as total FROM sotral_tickets WHERE status = 'used'",
        );
        ticketsMonth = await pool.query(
          "SELECT COUNT(*)::int as total FROM sotral_tickets WHERE purchased_at >= CURRENT_DATE - INTERVAL '30 days'",
        );
        totalRevenue = await pool.query(
          "SELECT COALESCE(SUM(price_paid_fcfa), 0)::int as total FROM sotral_tickets WHERE status != 'cancelled'",
        );
        revenueMonth = await pool.query(
          "SELECT COALESCE(SUM(price_paid_fcfa), 0)::int as total FROM sotral_tickets WHERE purchased_at >= CURRENT_DATE - INTERVAL '30 days' AND status != 'cancelled'",
        );
        totalPayments = await pool.query(
          "SELECT COUNT(*)::int as total FROM payment_receipts WHERE status = 'completed'",
        );
        paymentsMonth = await pool.query(
          "SELECT COUNT(*)::int as total FROM payment_receipts WHERE status = 'completed' AND created_at >= CURRENT_DATE - INTERVAL '30 days'",
        );
        activeLines = await pool.query(
          "SELECT COUNT(*)::int as total FROM sotral_lines WHERE is_active = true",
        );
        ...
        console.warn(
          "[AdminController.getDashboardStats] Tables SOTRAL non disponibles, utilisation de valeurs par défaut",
        );
      }
  ...
  static async getChartData(_req: AuthenticatedRequest, res: Response) {
    try {
      ...
      return res
        .status(400)
        .json({ success: false, error: "Type de graphique non supporté" });
    } catch (err: any) {
      console.error("[AdminController.getChartData]", err);
      return res
        .status(500)
        .json({ success: false, error: "Erreur chart data" });
    }
  }
  ...
  static async getRecentActivity(_req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const allActivities = [...ticketActivity.rows, ...validationActivity.rows]
        .sort(
          (a, b) =>
            new Date(b.time.split("/").reverse().join("-")).getTime() -
            new Date(a.time.split("/").reverse().join("-")).getTime(),
        )
        .slice(0, 10);
      ...
      try {
        const activities = await pool.query(
          "SELECT id, name as user, 'Nouvel utilisateur inscrit' as action, TO_CHAR(created_at, 'DD/MM/YYYY HH24:MI') as time FROM users ORDER BY created_at DESC LIMIT 10",
        );
        return res.status(200).json({ success: true, data: activities.rows });
      } catch (fallbackErr) {
        console.error(
          "[AdminController.getRecentActivity] Fallback error:",
          fallbackErr,
        );
        return res
          .status(500)
          .json({ success: false, error: "Erreur activité récente" });
      }
    }
  }

  // ---------- PAYMENTS & REPORTS ----------
  static async getPaymentStats(_req: AuthenticatedRequest, res: Response) {
    try {
      const s = await pool.query(
        "SELECT COUNT(*)::int as total, COALESCE(SUM(amount),0) as total_amount FROM payment_receipts",
      );
      return res.status(200).json({ success: true, data: s.rows[0] });
    } catch (err: any) {
      console.error("[AdminController.getPaymentStats]", err);
      return res
        .status(500)
        .json({ success: false, error: "Erreur stats paiements" });
    }
  }
  static async getAllPayments(_req: AuthenticatedRequest, res: Response) {
    try {
      const p = await pool.query(
        "SELECT * FROM payment_receipts ORDER BY created_at DESC LIMIT 100",
      );
      return res.status(200).json({ success: true, data: p.rows });
    } catch (err: any) {
      console.error("[AdminController.getAllPayments]", err);
      return res
        .status(500)
        .json({ success: false, error: "Erreur paiements" });
    }
  }
    ...
    try {
      ...
      if (type === "financial") {
        ...
        const averageTicketPrice =
          totalTickets.rows[0].total > 0
            ? Math.round(
                totalRevenue.rows[0].total / totalTickets.rows[0].total,
              )
            : 0;
      ...
      return res
        .status(400)
        .json({ success: false, error: "Type de rapport non supporté" });
    } catch (err: any) {
      console.error("[AdminController.getPeriodReport]", err);
      return res
        .status(500)
        .json({ success: false, error: "Erreur génération rapport" });
    }
  }
}
