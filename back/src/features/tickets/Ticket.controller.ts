...
// Ajout pour récupération des tickets de l'utilisateur connecté
...
export class TicketController {
  ...
  static async getRoutesByPriceCategory(req: Request, res: Response) {
    try {
      ...
      if (
        !category ||
        !["T100", "T150", "T200", "T250", "T300"].includes(category)
      ) {
        return res.status(400).json({
          success: false,
          error:
            "Catégorie de prix invalide. Utilisez T100, T150, T200, T250 ou T300.",
        });
      }
      ...
      console.error(
        "[TicketController.getRoutesByPriceCategory] error:",
        error,
      );
      return res.status(500).json({
        success: false,
  ...
  static async purchaseTicket(req: RequestWithUser, res: Response) {
    try {
      ...
      // Vérifier que la ligne existe s'il est spécifié
      let line = null;
      if (line_id) {
        line = await TicketRepository.getLineById(line_id);
        if (!line) {
          return res
            .status(404)
            .json({ success: false, error: "Ligne non trouvée" });
        }
        // Ici tu peux ajouter un contrôle de catégorie si besoin
      }
      ...
      return res
        .status(500)
        .json({ success: false, error: "Erreur lors de l'achat du ticket" });
    }
  }
  ...
  static async getUserTickets(req: RequestWithUser, res: Response) {
    try {
      ...
      if (!userId) {
        console.log(
          "[TicketController.getUserTickets] Utilisateur non authentifié",
        );
        return res
          .status(401)
      ...
      if (isNaN(userIdNumber)) {
        console.log(
          "[TicketController.getUserTickets] ID utilisateur invalide:",
          userId,
        );
        return res
          .status(400)
          .json({ success: false, error: "ID utilisateur invalide" });
      }
  ...
  static async validateTicket(req: RequestWithUser, res: Response) {
    try {
      ...
      if (!ticket) {
        return res
          .status(404)
          .json({ success: false, error: "Ticket non trouvé" });
      }
