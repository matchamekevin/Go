...
app.use(express.urlencoded({ extended: true })); // Permet de parser x-www-form-urlencoded (CinetPay)
...
app.use((req, res, next) => {
  ...
  if (method === "OPTIONS" || url.includes("/admin/")) {
    console.log(
      `🌐 CORS Request: ${method} ${url} from ${origin || "no-origin"}`,
    );
  }
...
if (process.env.NODE_ENV !== "production") {
  ...
  app.get("/_test/latest-email-otp", async (req: Request, res: Response) => {
    try {
      ...
      return res.json({
        success: true,
        data: { otp: record.otp, expires_at: record.expires_at },
      });
    } catch (e) {
      return res
        .status(500)
...
app.get("/register", (req: Request, res: Response) => {
  res
    ...
    .json({
      error:
        "Utilisez POST pour créer un compte. Endpoint recommandé: POST /auth/register",
    });
});
...
app.delete("/test-delete-ticket/:id", async (req: Request, res: Response) => {
  try {
    const { AdminSotralController } = await import(
      "./features/admin/admin.sotral.controller"
    );
...
app.use("/admin", (req: Request, res: Response) => {
  res
    .status(404)
    .json({ success: false, error: `Cannot ${req.method} ${req.originalUrl}` });
});
